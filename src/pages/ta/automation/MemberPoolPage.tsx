...
interface Member {
  ...
  position: string;
  rank: string;
  jobRole: string;              // 직무 (예: 개발자, 디자이너, 마케터)
  joinDate: Date;
  employeeId?: string;          // 임직원 ID
  lmsUserId?: number;           // LMS 사용자 ID
}
...
const sampleMembers: Member[] = [
  { id: 'm1', name: '김철수', email: 'cskim@company.com', department: '개발팀', position: '팀원', rank: '대리', jobRole: '백엔드 개발자', joinDate: new Date('2022-03-15'), employeeId: 'E001', lmsUserId: 101 },
  { id: 'm2', name: '이영희', email: 'yhlee@company.com', department: '개발팀', position: '팀원', rank: '과장', jobRole: '프론트엔드 개발자', joinDate: new Date('2020-08-01'), employeeId: 'E002', lmsUserId: 102 },
  { id: 'm3', name: '박지민', email: 'jmpark@company.com', department: '마케팅팀', position: '팀장', rank: '차장', jobRole: '마케팅 기획자', joinDate: new Date('2019-01-10'), employeeId: 'E003', lmsUserId: 103 },
  { id: 'm4', name: '최수진', email: 'sjchoi@company.com', department: '인사팀', position: '팀원', rank: '사원', jobRole: '인사 담당', joinDate: new Date('2024-01-02'), employeeId: 'E004', lmsUserId: 104 },
  { id: 'm5', name: '정민호', email: 'mhjung@company.com', department: '영업팀', position: '팀원', rank: '대리', jobRole: '영업 관리', joinDate: new Date('2021-06-15'), employeeId: 'E005', lmsUserId: 105 },
  { id: 'm6', name: '강서연', email: 'sykang@company.com', department: '개발팀', position: '팀원', rank: '사원', jobRole: '프론트엔드 개발자', joinDate: new Date('2023-09-01'), employeeId: 'E006', lmsUserId: 106 },
  { id: 'm7', name: '윤태희', email: 'thyun@company.com', department: '디자인팀', position: '팀장', rank: '과장', jobRole: 'UI/UX 디자이너', joinDate: new Date('2020-02-01'), employeeId: 'E007', lmsUserId: 107 },
  { id: 'm8', name: '임재현', email: 'jhlim@company.com', department: '경영지원팀', position: '팀원', rank: '대리', jobRole: '재무 담당', joinDate: new Date('2022-11-15'), employeeId: 'E008', lmsUserId: 108 },
  { id: 'm9', name: '한소희', email: 'shhan@company.com', department: 'QA팀', position: '팀원', rank: '과장', jobRole: 'QA 엔지니어', joinDate: new Date('2021-01-05'), employeeId: 'E009', lmsUserId: 109 },
  { id: 'm10', name: '오준영', email: 'jyoh@company.com', department: '개발팀', position: '팀원', rank: '부장', jobRole: '백엔드 개발자', joinDate: new Date('2018-05-20'), employeeId: 'E010', lmsUserId: 110 },
];
...
export const MemberPoolPage = () => {
  ...
  // 고유한 부서/직급/직책/직무 목록 추출
  const uniqueDepartments = Array.from(new Set(sampleMembers.map((m) => m.department))).sort();
  const uniqueRanks = Array.from(new Set(sampleMembers.map((m) => m.rank))).sort();
  const uniquePositions = Array.from(new Set(sampleMembers.map((m) => m.position))).sort();
  const uniqueJobRoles = Array.from(new Set(sampleMembers.map((m) => m.jobRole))).sort();
  ...
  const filteredMembers = sampleMembers.filter((member) => {
    ...
    const matchesSearch =
      ...
      member.jobRole.toLowerCase().includes(memberSearchTerm.toLowerCase());

    // 부서 필터
    const matchesDepartment =
      memberFilters.department === 'all' || member.department === memberFilters.department;

    // 직급 필터
    const matchesRank = memberFilters.rank === 'all' || member.rank === memberFilters.rank;

    // 직책 필터
    const matchesPosition =
      memberFilters.position === 'all' || member.position === memberFilters.position;

    // 직무 필터
    const matchesJobRole =
      memberFilters.jobRole === 'all' || member.jobRole === memberFilters.jobRole;

    return matchesSearch && matchesDepartment && matchesRank && matchesPosition && matchesJobRole;
  });
      ...
      <Dialog open={showMemberSelectModal} onOpenChange={setShowMemberSelectModal}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="py-4">
            ...
            {/* 필터 영역 */}
            <div className="mb-4 p-4 rounded-lg" style={{ backgroundColor: designTokens.bg.secondary }}>
              <div className="flex items-center gap-2 mb-3">
                <Filter className="w-4 h-4" style={{ color: designTokens.text.secondary }} />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {/* 부서 필터 */}
                <div>
                  <Label className="text-xs mb-1.5">부서</Label>
                  <Select
                    value={memberFilters.department}
                    onValueChange={(value) =>
                      setMemberFilters({ ...memberFilters, department: value })
                    }
                  >
                    <SelectTrigger>
                ...
                {/* 직급 필터 */}
                <div>
                  <Label className="text-xs mb-1.5">직급</Label>
                  <Select
                    value={memberFilters.rank}
                    onValueChange={(value) => setMemberFilters({ ...memberFilters, rank: value })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="전체" />
                    </SelectTrigger>
                ...
                {/* 직책 필터 */}
                <div>
                  <Label className="text-xs mb-1.5">직책</Label>
                  <Select
                    value={memberFilters.position}
                    onValueChange={(value) =>
                      setMemberFilters({ ...memberFilters, position: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="전체" />
                ...
                {/* 직무 필터 */}
                <div>
                  <Label className="text-xs mb-1.5">직무</Label>
                  <Select
                    value={memberFilters.jobRole}
                    onValueChange={(value) =>
                      setMemberFilters({ ...memberFilters, jobRole: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="전체" />
              ...
              {(memberFilters.department !== 'all' ||
                memberFilters.rank !== 'all' ||
                memberFilters.position !== 'all' ||
                memberFilters.jobRole !== 'all') && (
                <Button
                  variant="outline"
                  size="sm"
                  className="mt-3"
                ...
                <TableBody>
                  {filteredMembers.map((member) => (
                    <TableRow key={member.id}>
                      ...
                      <TableCell>
                        <Badge variant="blue">{member.rank}</Badge>
                      </TableCell>
                      <TableCell>
                        <span style={{ color: designTokens.text.secondary }}>
                          {member.position}
                      ...
                      <TableCell>
                        <Badge variant="purple">{member.jobRole}</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
